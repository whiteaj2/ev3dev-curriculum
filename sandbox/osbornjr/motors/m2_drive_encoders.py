...
Authors: David Fisher and Josh Osborne.
...
def main():
    ...
    while drive_time != 0:
        speed = int(input("Speed to travel(0 to 900dps): "))
        distance = int(input("Distance to travel(inches): "))
        if speed == 0 or distance == 0:
            break
        degrees_per_inch = 90
        motor_turns_needed = distance * degrees_per_inch
        left_motor.run_to_rel_pos(position_sp=motor_turns_needed, speed_sp = speed)
        right_motor.run_to_rel_pos(position_sp=motor_turns_needed, speed_sp = speed)
        left_motor.wait_while(left_motor.STATE_RUNNING)
